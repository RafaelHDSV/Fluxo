...
Use the `AskUserQuestion` tool for all choices. Free-form questions should be asked as text. Resolve ambiguities before moving on — if an answer is vague, drill in with follow-ups.
...
if [ -z "$SKILL_DIR" ]; then
  SKILL_DIR=$(dirname "$(find .claude/skills/md-to-wiki -name SKILL.md 2>/dev/null | head -1)")
fi
