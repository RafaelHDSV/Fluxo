...
**Out of scope:**
- Implementation proposal in `.issues/` (15 sections, per-file table, TS snippets) → rule `ai-development-workflow.mdc`
...
Overview → [this skill] → epic body on GitHub (full spec)
       → epic approval
       → increments / partials → ai-development-workflow.mdc → .issues/
```
...
3. One line: *"Para implementar um incremento desta epic, use `ai-development-workflow.mdc` e gere `.issues/` — não implemente código neste fluxo."*
...
- Duplicate the 15 sections of `ai-development-workflow.mdc`.
