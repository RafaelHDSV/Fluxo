...
Follow [git-github.mdc](../../rules/git-github.mdc) and [pr-review-readonly.mdc](../../rules/pr-review-readonly.mdc) — **read-only security lock** applies unless the user explicitly overrides travas.
...
```
PR target(s) → MCP GitHub (user-Toolbox)
...
| 2 (only if `gh` missing / auth fail) | MCP `user-Toolbox` → `github__search_pull_requests` | `query: "is:open review-requested:@me"` as last resort |
...
Reading PR, issue, diff, files, and checks **starts** with **MCP GitHub** on **`user-Toolbox`**. On **404/422 permission** → **0.1**, then **0.2** before `insuficiente`.

| Rule | Detail |
|------|--------|
| **MCP server** | `user-Toolbox` |
| **Invocation** | `CallMcpTool` / Toolbox `execute` |
...
- `docs/context.md` if present (`ai-context.mdc`)
...
| Artifact | Relation |
|----------|----------|
| `git-github.mdc` | MCP → 0.1 → 0.2; no GitHub writes |
| `pr-review-readonly.mdc` | Create or update one file per PR; no GitHub publish |
| `security-secrets.mdc` | Secret in diff → blocker (+ Segurança section) |
| `ai-context.mdc` | `docs/context.md` |
| `ai-development-workflow.mdc` | `.issues/` = implement; `.reviews/` = review |
